...
class RevealServer(IPyPostProcessor):
    ...
    def run_postprocess(self, stream, mimetype, filepath, resources):
        ...
        if not filepath.exists():
            self.handle_error('the target file path does not exist: {}'.format(filepath), IOError)
        ...
        if port_attempt == port_attempts[-1]:
            self.handle_error('no port available to launch slides on, ' 'try closing some slideshows', IOError)
