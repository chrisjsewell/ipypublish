...
class RevealServer(IPyPostProcessor):
    ...
    reveal_cdn = Unicode(
        'https://cdnjs.cloudflare.com/ajax/libs/reveal.js/3.1.0', help="""URL for reveal.js CDN.""").tag(config=True)
    ...
    def run_postprocess(self, stream, mimetype, filepath, resources):
        ...
        if not filepath.exists():
            self.handle_error('the target file path does not exist: {}'.format(filepath), klass=IOError)
        ...
        if '://' in self.reveal_prefix or self.reveal_prefix.startswith('//'):
            ...
            self.logger.info('Redirecting %s requests to %s', self.reveal_prefix, self.reveal_cdn)
            self.logger.info('Redirecting %s requests to %s', self.reveal_prefix, self.reveal_cdn)
        ...
        if port_attempt == port_attempts[-1]:
            self.handle_error('no port available to launch slides on, ' 'try closing some slideshows', klass=IOError)
