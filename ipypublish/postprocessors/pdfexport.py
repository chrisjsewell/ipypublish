...
class PDFExport(IPyPostProcessor):
    ...
    files_folder = Unicode(
        '_static', help='the path (relative to the main file path) '
        'containing external files').tag(config=True)

    convert_in_temp = Bool(
        False, help='run conversion in a temporary directory, '
        'and copy back only PDF file').tag(config=True)

    debug_mode = Bool(False, help='run in debug mode').tag(config=True)

    open_in_browser = Bool(False, help='launch a html page containing a pdf browser').tag(config=True)
    ...
    def _export_pdf(self, texpath):
        ...
        if not texpath.exists():
            self.handle_error(msg='the target file path does not exist: {}'.format(texpath), klass=IOError)
        ...
        if external_files.exists() and not external_files.is_dir():
            self.handle_error('the external folder path is not a directory: {}'.format(external_files), klass=IOError)
        ...
        if self.convert_in_temp:
            ...
            try:
                ...
                if exitcode == 0:
                    shutil.copyfile(
                        os.path.join(out_folder, texname + '.pdf'), str(texpath.parent.joinpath(texname + '.pdf')))
            finally:
        ...
        if exitcode == 0:
            ...
            self.handle_error('pdf conversion failed: ' 'Try running with pdf-debug flag', klass=RuntimeError)
    ...
    def _run_latexmk(self, texpath, out_folder, external_files):
        ...
        if os.path.dirname(str(texpath)) != str(out_folder):
            self.logger.debug('copying tex file to: {}'.format(os.path.join(str(out_folder), texpath.name)))
