...
class RunSphinx(IPyPostProcessor):
    ...
    open_in_browser = Bool(True, help='launch a html page containing a pdf browser').tag(config=True)

    numbered = Bool(True, help='set :numbered: in toc, which numbers sections, etc').tag(config=True)

    show_prompts = Bool(True, help='whether to include cell prompts').tag(config=True)

    prompt_style = Unicode('[{count}]:', help='the style of cell prompts').tag(config=True)
    ...
    def _valid_prompt_style(self, proposal):
        try:
            ...
            raise TraitError('prompt_style should be formatable by ' '`prompt_style.format(count=1)`')
        return proposal['value']

    conf_kwargs = Dict(help=('additional key-word arguments to be included in the conf.py '
                             'as <key> = <val>')).tag(config=True)

    override_defaults = Bool(True, help='if True, conf_kwargs override default values').tag(config=True)
    ...
    def run_postprocess(self, stream, mimetype, filepath, resources):
        ...
        try:
            ...
            self.handle_error(exception=err)
        ...
        if self.open_in_browser and not exitcode:
            ...
            entry_path = entry_path.joinpath(os.path.splitext(filepath.name)[0] + '.html')
            if entry_path.exists():
                #  2 opens the url in a new tab
                webbrowser.open(entry_path.as_uri(), new=2)
            else:
                self.handle_error("can't find {0} to open".format(entry_path), klass=IOError)
