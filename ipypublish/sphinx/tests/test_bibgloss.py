...
def test_bibgloss_basic(app, status, warning, get_sphinx_app_output, data_regression):
...
@pytest.mark.sphinx(buildername='html', srcdir=get_test_source_dir('bibgloss_missingref'))
def test_missingref(app, status, warning, get_sphinx_app_output):
    ...
    assert 'build succeeded' in status.getvalue()  # Build succeeded
    warnings = warning.getvalue().strip()
    if ("could not relabel bibglossary reference [missingkey]" not in warnings and  # sphinx < 2
            "WARNING: citation not found: missingkey" not in warnings):  # sphinx >= 2
        raise AssertionError("should raise warning for missing citation `missingkey`: {}".format(warnings))


@pytest.mark.sphinx(buildername='html', srcdir=get_test_source_dir('bibgloss_duplicatekey'))
...
@pytest.mark.skipif(sys.version_info < (3, 0), reason="SyntaxError on import of texsoup/data.py line 135")
