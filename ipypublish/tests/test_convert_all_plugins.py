...
@pytest.mark.parametrize('plugin_name', [f.name for f in iter_all_export_files()])
@pytest.mark.ipynb('basic_nb')
def test_basic_all_plugins(ipynb_app, plugin_name):

    if ((plugin_name in ['sphinx_ipypublish_all.ext'] or 'exec' in plugin_name) and sys.version_info[0] < 3):
    ...
    if plugin_name in ['python_with_meta_stream', 'sphinx_ipypublish_all.ext', 'sphinx_ipypublish_all.ext.noexec']:
        return

    # test build exists only
    if plugin_name in ['sphinx_ipypublish_all.run', 'sphinx_ipypublish_main.run']:
        outpath = ipynb_app.converted_path.joinpath('build', 'html',
