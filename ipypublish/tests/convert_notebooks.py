...
def convert_all(inpath, outpath):
    ...
    for export_file in iter_all_export_files():

        out_folder = tempfile.mkdtemp()
        publish = IpyPubMain(config={'IpyPubMain': {'conversion': export_file.name, 'outpath': out_folder}})
        try:
            ...
            if not os.path.exists(outfile):
                raise IOError('could not find: {} for {}'.format(outfile, export_file.name))

            shutil.copyfile(outfile, os.path.join(str(outpath), export_file.name + extension))
