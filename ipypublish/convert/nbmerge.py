...
def merge_notebooks(ipynb_path, ignore_prefix='_', to_str=False, as_version=4):
    ...
    if not ipynb_path.exists():
        handle_error(msg='the notebook path does not exist: {}'.format(ipynb_path), klass=IOError, logger=logger)
    ...
    if final_nb is None:
        handle_error(
            msg='no acceptable notebooks found for path: {}'.format(ipynb_path.name), klass=IOError, logger=logger)
