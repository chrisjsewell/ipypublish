...
class IpyPubMain(Configurable):
    ...
    plugin_folder_paths = T.Set(
        T.Unicode(), default_value=(), help='a list of folders containing conversion configurations').tag(config=True)
    ...
    pre_conversion_funcs = T.Dict(
        help=('a mapping of file extensions to functions that can convert'
              'that file type Instance(nbformat.NotebookNode) = func(pathstr)')).tag(config=True)
    ...
    log_stdout_formatter = T.Type(default_value=logging.Formatter, klass=logging.Formatter).tag(config=True)
    ...
    log_file_path = T.Unicode(
        None, allow_none=True, help='if None, will output to {outdir}/{ipynb_name}.nbpub.log').tag(config=True)

    log_file_formatstr = T.Unicode('%(levelname)s:%(name)s:%(message)s').tag(config=True)

    log_file_formatter = T.Type(default_value=logging.Formatter, klass=logging.Formatter).tag(config=True)

    default_ppconfig_kwargs = T.Dict(
        trait=T.Bool(),
        default_value=(('pdf_in_temp', False), ('pdf_debug', False), ('launch_browser', False)),
    ...
    default_pporder_kwargs = T.Dict(
        trait=T.Bool(),
        default_value=(('dry_run', False), ('clear_existing', False), ('dump_files', False), ('create_pdf', False),
    ...
    def _setup_logger(self, ipynb_name, outdir):
        ...
        if self.log_to_file:
            ...
            fformatter = self.log_file_formatter(self.log_file_formatstr)
            flogger.setFormatter(fformatter)
        ...
        if self.log_to_stdout:
            ...
            sformatter = self.log_stdout_formatter(self.log_stdout_formatstr)
            slogger.setFormatter(sformatter)
